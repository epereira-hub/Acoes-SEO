/* =========================================================================
  ...
  const STATUS_MAP = {
    ...
    'aguardando aprovacao': 'Em aprovação',
    'em validacao': 'Em validação',
    'validacao': 'Em validação',
    'aguardando validacao': 'Em validação',
    'aguardando insumos': 'Aguardando insumos',
    'aguardando insumo': 'Aguardando insumos',
    'aguardando': 'Aguardando insumos',
    'pausado': 'Pausada',
    'pausada': 'Pausada',
    'pausadas': 'Pausada',
    'em pausa': 'Pausada',
  ...
  function computeKpis(data) {
    ...
    const emAprovacao = data.filter((d) => d.status === 'Em aprovação').length;
    const emValidacao = data.filter((d) => d.status === 'Em validação').length;
    const aguardandoInsumos = data.filter((d) => d.status === 'Aguardando insumos').length;
    ...
    // Pausadas não entram na base de cálculo do percentual de conclusão.
    const effectiveTotal = total - paused;
    const pct = effectiveTotal ? Math.round((done / effectiveTotal) * 100) : 0;
  
    return {
      total, done, inProgress, emAprovacao, emValidacao, aguardandoInsumos, paused,
      pending, blocked, highImpact, highPriority, effectiveTotal, pct,
    };
  }
  ...
  function computeProductBreakdown(data) {
    ...
    return products.map((product) => {
      ...
      const paused = items.filter((d) => d.status === 'Pausada').length;
      const highImpact = items.filter((d) => d.impact === 'Alto').length;
      // Pausadas não entram na base de cálculo do percentual de conclusão.
      const effectiveTotal = total - paused;
      const pct = effectiveTotal ? Math.round((done / effectiveTotal) * 100) : 0;
      return { product, total, done, inProgress, pending, paused, highImpact, effectiveTotal, pct };
    }).sort((a, b) => b.total - a.total);
  }
  
  function computeStatusDistribution(data) {
    const order = ['Concluído', 'Em andamento', 'Em aprovação', 'Em validação', 'Aguardando insumos', 'Pausada', 'Pendente', 'Bloqueado'];
  ...
  function computeCriticalActions(data) {
    const items = data.filter((d) => d.impact === 'Alto' && d.priority === 'Alta');
    const total = items.length;
    const done = items.filter((d) => d.status === 'Concluído').length;
    const paused = items.filter((d) => d.status === 'Pausada').length;
    // Pausadas não entram na base de cálculo do percentual de conclusão.
    ...
    const pct = effectiveTotal ? Math.round((done / effectiveTotal) * 100) : 0;
    const order = ['Concluído', 'Em andamento', 'Em aprovação', 'Em validação', 'Aguardando insumos', 'Pausada', 'Pendente', 'Bloqueado'];
    const found = [...new Set(items.map((d) => d.status))];
    const statuses = [...order.filter((s) => found.includes(s)), ...found.filter((s) => !order.includes(s))];
    const byStatus = statuses.map((status) => ({
      status,
      count: items.filter((d) => d.status === status).length,
    }));
    return { total, done, paused, effectiveTotal, pct, byStatus, items };
  }
