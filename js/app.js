...
const STATE = {
  raw: [],
  filtered: [],
  filters: { produto: '', mes: '', status: '', prioridade: '' },
...
function bindStaticEvents() {
  ...
  ['produto', 'mes', 'status', 'prioridade'].forEach((key) => {
...
function populateFilterOptions(data) {
  const config = [
    ['produto', 'product'], ['mes', 'month'], ['status', 'status'], ['prioridade', 'priority'],
  ];
...
function clearFilters() {
  STATE.filters = { produto: '', mes: '', status: '', prioridade: '' };
...
function renderKpis(data) {
  ...
  el.innerHTML = [
    ...
    kpiCard('Em aprovação', k.emAprovacao, null, 'accent'),
    kpiCard('Em validação', k.emValidacao, null, 'neutral'),
    kpiCard('Aguardando insumos', k.aguardandoInsumos, null, 'neutral'),
    kpiCard('Pausadas', k.paused, null, 'paused'),
  ].join('');
...
function renderCriticalActions(data) {
  const c = computeCriticalActions(data);
  const el = document.getElementById('critical-cards');
  const progressWrap = document.getElementById('critical-progress-wrap');

  if (!c.total) {
    el.innerHTML = '<p class="empty-hint">Nenhuma ação de alto impacto e alta prioridade para os filtros atuais.</p>';
    progressWrap.hidden = true;
    return;
  }

  progressWrap.hidden = false;
  const statusCards = c.byStatus
    .filter((s) => s.status !== 'Concluído')
    .map((s) => kpiCard(s.status, s.count, null, 'neutral'));

  el.innerHTML = [
    kpiCard('Ações críticas', c.total, null, 'accent'),
    kpiCard('Concluídas', c.done, `${c.pct}%`, 'success'),
    ...statusCards,
  ].join('');

  document.getElementById('critical-progress-bar').style.width = `${c.pct}%`;
  document.getElementById('critical-progress-label').textContent = `${c.pct}% concluído`;
}
  ...
  el.innerHTML = breakdown.map((p) => `
    <div class="product-card">
      ...
      <ul class="product-card__stats">
        ...
        ${p.paused ? `<li><span class="dot dot--paused"></span>${p.paused} pausadas</li>` : ''}
      </ul>
    </div>`).join('');
}
